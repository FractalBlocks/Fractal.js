...
let def = moduleDef => {
  ...
  if (mDef.animations) {
    ns.animations = style.registerAnimations(mDef.styleInstance.Style, mDef.animations)
  }
  if (mDef.styles) {
    mDef.styles = style.rs(mDef.styleInstance, (typeof(mDef.styles === 'function')) ? mDef.styles(ns) : mDef.styles)
    mDef.dispose = () => {
