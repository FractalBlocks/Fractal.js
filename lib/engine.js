...
let run = engineDef => {
  ...
  let attach = model => { // model is used for webpack HMR
    ...
    model$ = flyd.scan((m, a) => log(m, middleUpdates(module.update(a, m)), Composing.addModuleInfo(module, a)), log({}, middleUpdates((model) ? model : module.init({key: 'mainModule'}))), ctx.action$) // state
