...
import { logAction, displayFromDiff, diff } from './utils/logdiff'
...
let run = engineDef => {
  ...
  let attach = model => { // model is used for webpack HMR
    ...
    function log(m, newModel, action = { _initial: true, _log: engineDef.log || engineDef.logAll, _modulePath: module.name }) {
      if (engineDef.log && action._log || engineDef.logAll) {
        if (!action._initial) {
          logAction(action)
          displayFromDiff(diff(m, newModel))
        } else {
          console.log('%c Fractal is initializing your app, model computed ... have a nice day :)', 'color: purple; font-size: 14px')
        }
