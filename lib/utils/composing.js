...
let connectInterface = (md, name, model, connections) =>
  (md.interfaces[name]) ? merge(md).partial(connections).interfaces[name](model) : {}
...
let mergeModels = mds => {
  let obj = {}
  for (let key in mds) {
    obj[key] = (mds[key].root) ? mds[key].root.init({key, ...mds[key]}) : mds[key].init({key})
  }
  return obj
}
...
let mergeChildList = (childNames, interfaceName, f = interfaceObj => interfaceObj, _md, rootModel) =>
 R.map(name => f(_md[name].interfaces[interfaceName](rootModel[name])), childNames)
...
let mergeAll = (childDefs, i, scope = '', connectionsFn = (md, name) => ({})) => {
  ...
  return R.mapObjIndexed((md, name) => merge((md.root) ? md.root : md, connectionsMiddleFn(connectionsFn(md, name), md, name)), childDefs)
}

let mergeDynamicList = (scope, i, mds, interfaceName, interfaceFn, m) => {
  return m[scope].map((item, idx) => {
    return interfaceFn(merge(mds.dynamicModules[scope], {action$: i._dynamicChildAction(scope, mds.dynamicModules[scope].update, idx)}).interfaces[interfaceName](item), idx)
  })
}
