...
function addModuleInfo(mDef, obj) {
  obj._modulePath = (obj.hasOwnProperty('_modulePath')) ? obj._modulePath + '->' + mDef.name : mDef.name
  obj._log = mDef.log
  return obj
}
...
let createContext = (mDefinition, connections = {}) => {
  ...
  for (let name in connections) {
    if (!!ctx[name]) {
      if (name === 'action$') {
        flyd.on(addModuleInfo(connections[name]), ctx[name])
      } else {
  ...
  function connectTasks(mds) {
    for (let name in mds) {
      if (mds[name].mDef) {
        flyd.on(task => ctx.task$(addModuleInfo(task)), mds[name].ctx.task$)
      } else {
        // connect categorized modules
        for (let subName in mds[name]) {
          flyd.on(task => ctx.task$(addModuleInfo(task)), mds[name][subName].ctx.task$)
        }
      }
    }
  }
